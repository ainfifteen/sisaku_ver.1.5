...
public class EdgeServer {
	...
	double dvisionAreaDistance[][] = new double[4][4];
	...
	void receiveData(int initX, int initY) throws IOException{//受信メソッド
		...
		byte[] rcvDataSecond = udps.lisnersecond.getData();
	...
	void divisionArea(int x, int y) {
		...
		for(int i = 0 ; i < 4 ; i++) {
			...
			divisionArea[i][0] = X;
			divisionArea[i][1] = Y;
			System.out.println((i+1) +" "+divisionArea[i][0]+ " " + divisionArea[i][1]);
		}
	}
	...
	void calcAreaDistance() {
		...
		for(int i = 0; i < 4; i++) {
			for(int k = 0; k < 4; k ++) {
				System.out.println((i + 1) + " " + dvisionAreaDistance[k][i]);
		    }
